...
class PilferTestBase(unittest.TestCase, ABC):
    ...
    def test_unchanged_files_same_hash(self):
        ...
        self.assertEqual(modified_count, 0, "No files should be marked as modified when unchanged")
    ...
    def test_line_endings_preserved(self):
        ...
        with open('windows_vault.yml', 'rb') as f:
            decrypted_content = f.read()
            self.assertEqual(decrypted_content, self.vault_content_windows.encode('utf-8'))
        
        with open('mixed_vault.yml', 'rb') as f:
            decrypted_content = f.read()
            self.assertEqual(decrypted_content, self.vault_content_mixed.encode('utf-8'))
...
class TestCompatibility(unittest.TestCase):
    ...
    def setUp(self):
        ...
        vault = VaultLib([
            (DEFAULT_VAULT_ID_MATCH, VaultSecret(b'test_password'))
        ])
    ...
    def test_identical_behavior(self):
        ...
        self.assertEqual(cli_modified_count, 0, "CLI should report 0 modifications for unchanged content")
